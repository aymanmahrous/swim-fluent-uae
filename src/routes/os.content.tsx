...
async function generateCampaign(input: typeof campaignDefaults) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      error.success && error.data.code ? error.data.code : "CONTENT_GENERATION_FAILED",
    );
  }
...
function errorMessage(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    STORED_PROVIDER_NOT_READY: "مزوّد مهمة الفيديو الأصلية غير جاهز على الخادم.",
...
function ContentPage() {
  ...
  useEffect(() => {
    if (videoJobId || !activeVideoJob) return;
    setVideoJobId(activeVideoJob.jobId);
    setVideoPrompt(activeVideoJob.prompt);
    setMatchingMedia({ type: "video", prompt: activeVideoJob.prompt });
    setMediaMode("video");
  }, [activeVideoJob, videoJobId]);
  ...
  const videoCreation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["os", "video-jobs"] });
    },
    onError: (error) => toast.error(errorMessage(error)),
  });
  ...
  function resumeVideoJob(job: (typeof videoJobs)[number]) {
    setVideoJobId(job.jobId);
    setVideoPrompt(job.prompt);
    setMatchingMedia({ type: "video", prompt: job.prompt });
    setNotifiedVideoState("");
    setMediaMode("video");
  }
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">
            Real Qwen/Wan provider adapters, permanent media storage and human review. No
            provider is represented as executable until its server credentials are configured.
          </p>
        </div>
        <Link
          to="/os/media"
          className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-3 text-sm font-black"
        >
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-3">
        {[
          [
            "text-ai",
            WandSparkles,
            "Campaign text engine",
            "Creates structured hooks, captions, CTAs, hashtags and visual prompts, then saves them as needs_review.",
            textReady,
          ],
          [
            "image-ai",
            Image,
            "AI image generation",
            "Generates one Wan image and immediately copies temporary provider output into permanent Supabase Storage.",
            imageReady,
          ],
          [
            "video-ai",
            Video,
            "AI video generation",
            "Creates a Wan async text-to-video or image-to-video task, resumes pending jobs and persists successful output to Media Library.",
            videoReady,
          ],
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <span
                  className={`rounded-full px-3 py-1 text-xs font-black ${ready ? "bg-success/10 text-success" : "bg-gold/15 text-gold-foreground"}`}
                >
        ...
        <div className="flex flex-col gap-4 border-b border-border pb-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Generate, persist and catalog the asset before using it in publishing workflows.
            </p>
          </div>
          <div className="flex rounded-xl border border-border bg-muted p-1">
            <button
              onClick={() => setMediaMode("image")}
              className={`rounded-lg px-4 py-2 text-sm font-black ${mediaMode === "image" ? "bg-card text-primary shadow" : "text-muted-foreground"}`}
            >
              <Image className="me-2 inline h-4 w-4" /> Image
            </button>
            <button
              onClick={() => setMediaMode("video")}
              className={`rounded-lg px-4 py-2 text-sm font-black ${mediaMode === "video" ? "bg-card text-primary shadow" : "text-muted-foreground"}`}
            >
              <Video className="me-2 inline h-4 w-4" /> Video
            </button>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <label className="block text-sm font-black">
              ...
              <select
                ...
                className="mt-2 w-full rounded-xl border border-border bg-input px-4 py-3 font-normal"
              >
                <option value="">Standalone media asset</option>
                {contentItems.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.platform} · {item.topic || item.contentType} · {item.status}
                  </option>
                ))}
              </select>
            </label>
              ...
              <>
                ...
                <label className="block text-sm font-black">
                  ...
                  <select
                    ...
                    className="mt-2 w-full rounded-xl border border-border bg-input px-4 py-3 font-normal"
                  >
                    {(["1:1", "4:5", "5:4", "9:16", "16:9", "3:4", "4:3"] as const).map(
                      (ratio) => (
                ...
                <button
                  disabled={!imageReady || imageGeneration.isPending || imagePrompt.trim().length < 2}
                  onClick={() =>
                  ...
                  className="w-full rounded-xl gradient-aqua px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {imageGeneration.isPending
                    ? "Generating and saving..."
                    : "Generate image & save permanently"}
                </button>
              </>
            ) : (
              <>
                ...
                <label className="block text-sm font-black">
                  Source image URL (optional — enables image-to-video)
                  <input
                    type="url"
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block text-sm font-black">
                    ...
                    <select
                      value={videoRatio}
                      onChange={(event) => setVideoRatio(event.target.value as typeof videoRatio)}
                      className="mt-2 w-full rounded-xl border border-border bg-input px-4 py-3 font-normal"
                    >
                      {(["9:16", "16:9", "1:1", "4:3", "3:4"] as const).map((ratio) => (
                        <option key={ratio}>{ratio}</option>
                      ))}
                    </select>
                  </label>
                ...
                <button
                  disabled={!videoReady || videoCreation.isPending || videoPrompt.trim().length < 2}
                  onClick={() =>
                    videoCreation.mutate({
                      prompt: videoPrompt,
                      sourceAssetUrl: videoSourceUrl.trim() || null,
                  ...
                  className="w-full rounded-xl gradient-aqua px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {videoCreation.isPending ? "Creating video task..." : "Generate video & track job"}
                </button>

                {videoJobId && (
                  <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm">
                    <div className="font-black">
                      Video job: {videoJobQuery.data?.status ?? "checking"}
                    </div>
                    <div className="mt-1 break-all text-xs text-muted-foreground">{videoJobId}</div>
                    {videoJobQuery.isError && (
                      <div className="mt-2 text-destructive">
                        {errorMessage(videoJobQuery.error)}
                      </div>
                    )}
                  </div>
                )}
                ...
                {videoJobs.length > 0 && (
                  <div className="rounded-xl border border-border bg-muted/30 p-4">
                    <div className="text-sm font-black">Recent video jobs</div>
                    <div className="mt-3 grid gap-2">
                      {videoJobs.slice(0, 5).map((job) => (
                        <button
                          key={job.jobId}
                          type="button"
                          onClick={() => resumeVideoJob(job)}
                          className="flex items-center justify-between gap-3 rounded-lg border border-border bg-card px-3 py-2 text-start text-xs"
                        >
                          <span className="min-w-0 truncate">{job.prompt}</span>
                          <span className="shrink-0 font-black text-primary">{job.status}</span>
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          <div className="min-h-[360px] overflow-hidden rounded-2xl border border-border bg-muted/40">
            {latestMediaUrl ? (
              matchingMedia?.type === "video" ? (
                ...
                <img
                  src={latestMediaUrl}
                  alt="Generated Relax Fix media"
                  className="h-full min-h-[360px] w-full object-contain"
                />
              )
            ) : (
              <div className="grid min-h-[360px] place-items-center p-8 text-center text-sm text-muted-foreground">
                <div>
                  {mediaMode === "image" ? (
                  ...
                  <p className="mt-4">
                    The permanent generated asset will appear here after provider execution and
      ...
      <section className="mt-8 rounded-2xl border border-border bg-card p-6 shadow-elegant">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-xl font-black">Generate matching copy for the visual</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Qwen receives the exact media-generation prompt and creates copy aligned to that
              visual concept without pretending to inspect pixels or frames.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <select
              value={copyPlatform}
              onChange={(event) => setCopyPlatform(event.target.value as Platform)}
            ...
            <select
              value={copyLanguage}
              onChange={(event) => setCopyLanguage(event.target.value as "ar" | "en")}
              className="rounded-xl border border-border bg-input px-3 py-2 text-sm"
            >
        ...
        <input
          ...
          className="mt-4 w-full rounded-xl border border-border bg-input px-4 py-3 text-sm"
          placeholder="Content goal"
        />
        <button
          disabled={!textReady || !matchingMedia || matchingCopy.isPending}
          onClick={() =>
            matchingMedia &&
            matchingCopy.mutate({
              assetType: matchingMedia.type,
              mediaPrompt: matchingMedia.prompt,
              platform: copyPlatform,
              language: copyLanguage,
              goal: copyGoal,
            })
          }
          className="mt-4 inline-flex items-center gap-2 rounded-xl bg-deep px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
        >
          <Sparkles className="h-4 w-4" />
          {matchingCopy.isPending ? "Writing..." : "Generate matching hook, caption & CTA"}
        </button>

        {matchingCopy.data && (
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            <div className="rounded-xl border border-border bg-muted/30 p-5">
              <div className="text-xs font-black uppercase text-primary">Hook</div>
              <p className="mt-2 font-black">{matchingCopy.data.copy.hook}</p>
              <div className="mt-5 text-xs font-black uppercase text-primary">CTA</div>
              <p className="mt-2">{matchingCopy.data.copy.cta}</p>
              <div className="mt-5 text-xs font-black uppercase text-primary">Hashtags</div>
              <p className="mt-2 text-sm text-muted-foreground">
                {matchingCopy.data.copy.hashtags.join(" ")}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-muted/30 p-5">
              <div className="flex items-center justify-between gap-3">
                <div className="text-xs font-black uppercase text-primary">Caption</div>
                <button
                  onClick={() =>
                    void navigator.clipboard.writeText(
                      `${matchingCopy.data.copy.hook}\n\n${matchingCopy.data.copy.caption}\n\n${matchingCopy.data.copy.cta}\n\n${matchingCopy.data.copy.hashtags.join(" ")}`,
                    )
                  }
      ...
      <section className="mt-8 rounded-2xl border border-border bg-card p-6 shadow-elegant">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 className="text-xl font-black">Generate a real 30-day campaign</h2>
            <p className="mt-2 max-w-2xl text-sm leading-7 text-muted-foreground">
              The text provider must return validated structured JSON. The server persists the full
              batch atomically as <code>needs_review</code>; publishing is not implied.
            </p>
          </div>
          <Link
            to="/os/planner"
          ...
          >
            <CalendarDays className="h-4 w-4" /> Review planner
          </Link>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <label className="text-sm font-black">
            ...
            <input
              value={campaign.goal}
              onChange={(event) =>
                setCampaign((current) => ({ ...current, goal: event.target.value }))
              }
              className="mt-2 w-full rounded-xl border border-border bg-input px-4 py-3 font-normal"
            />
          </label>
          ...
          <label className="text-sm font-black md:col-span-2">
            ...
            <input
              ...
              onChange={(event) =>
                setCampaign((current) => ({ ...current, topic: event.target.value }))
              }
              className="mt-2 w-full rounded-xl border border-border bg-input px-4 py-3 font-normal"
            />
          </label>
          <label className="text-sm font-black">
            ...
            <input
              ...
              value={campaign.days}
              onChange={(event) =>
                setCampaign((current) => ({ ...current, days: Number(event.target.value) }))
              }
              className="mt-2 w-full rounded-xl border border-border bg-input px-4 py-3 font-normal"
            />
          </label>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <button
            disabled={!textReady || generation.isPending}
            onClick={() => generation.mutate(campaign)}
            className="inline-flex items-center gap-2 rounded-xl gradient-aqua px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
            <WandSparkles className="h-4 w-4" />
            {generation.isPending
              ? "Generating and validating..."
              : `Generate ${campaign.days}-day campaign`}
          </button>
          <Link
            to="/os/integrations"
            className="inline-flex items-center gap-2 text-sm font-black text-primary"
          >
            Provider readiness <ExternalLink className="h-4 w-4" />
          </Link>
        </div>
      </section>
    </div>
