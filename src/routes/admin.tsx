...
function Admin() {
  ...
  function login(e: React.FormEvent) {
    e.preventDefault();
    if (pass === PASSWORD) {
      sessionStorage.setItem("rfx_admin", "1");
      setAuthed(true);
      setErr(false);
    } else setErr(true);
  }
  ...
  if (!authed) {
    return (
      <div dir={dir} className="min-h-[70vh] grid place-items-center px-6">
        <form
          onSubmit={login}
          className="w-full max-w-sm p-8 rounded-2xl bg-card border border-border shadow-elegant"
        >
  ...
  return (
    <div dir={dir} className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          onClick={() => {
            sessionStorage.removeItem("rfx_admin");
            setAuthed(false);
          }}
      ...
      <section className="mb-8 p-6 rounded-2xl bg-card border border-border shadow-elegant">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          ...
          <div className="flex items-end">
            <button
              onClick={saveCfg}
              className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-semibold inline-flex items-center justify-center gap-2"
            >
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {cfg.locations.map((l) => (
            ...
            >
              ...
              <button
                onClick={() => {
                  const c = { ...cfg, locations: cfg.locations.filter((x) => x !== l) };
                  setCfg(c);
                  saveConfig(c);
                }}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              ...
              const c = { ...cfg, locations: [...cfg.locations, newLoc.trim()] };
              setCfg(c);
              saveConfig(c);
              setNewLoc("");
            }}
            ...
            <tbody>
              {bookings.map((b) => (
                <tr key={b.id} className="border-b border-border/50 hover:bg-muted/30">
                  <td
                    className={`p-2 font-bold ${b.fearOfWater ? "text-destructive" : "text-muted-foreground"}`}
                  >
                  ...
                  <td className="p-2">
                    {b.location === "Other" ? `${b.location}: ${b.otherLocation}` : b.location}
                  </td>
                  ...
                  <td className="p-2 flex gap-1">
                    <select
                      value={b.status}
                      onChange={(e) => {
                        updateBooking(b.id, { status: e.target.value as Booking["status"] });
                        refreshBookings();
                      }}
