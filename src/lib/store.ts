...
export const defaultConfig: Config = {
  price: 150,
  duration: 45,
  locations: ["Al Muroor", "Al Ma'amoor", "Al Khalidiya", "Al Falah", "Electra", "Al Reem Island", "Yas Island"],
};
...
export function saveConfig(c: Config) { localStorage.setItem(CF, JSON.stringify(c)); }
...
export function generateSlots(date: Date, duration = 45): string[] {
  ...
  if (day === 1) ranges.push([16, 21]); // Mon 4pm-9pm
  else if (day === 0 || day === 6) ranges.push([18, 22]); // Sat/Sun 6pm-10pm
  else if (day === 2 || day === 4 || day === 5) { ranges.push([13, 15]); ranges.push([19, 22]); }
