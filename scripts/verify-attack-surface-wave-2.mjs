...
for (const file of routeFiles) {
  ...
  if (file.startsWith("api.os-") || file.startsWith("api.staff-")) {
    assert.ok(source.includes("resolveStaffSession(request)"), `${file} must resolve a Staff session before serving protected data`);
    continue;
  }
...
for (const contract of ["MAX_PROVIDER_ASSET_BYTES", "allowedContentType", "PROVIDER_ASSET_HOST_NOT_ALLOWLISTED", 'redirect: "manual"', '"x-upsert": "false"', "encodedObjectPath"]) {
  assert.ok(mediaStorage.includes(contract), `Media storage must retain ${contract}`);
}
assert.ok(mediaStorage.includes('url.protocol !== "https:"') && mediaStorage.includes("isIP(url.hostname)"), "Remote media downloads must remain HTTPS-only and reject literal IP hosts");
...
assert.ok(!imageRoute.includes('message.split(":")[0].replace') || imageRoute.includes("PUBLIC_ERROR_CODES.has(candidate)"), "Provider messages must not become arbitrary public error codes");
