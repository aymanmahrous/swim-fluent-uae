...
const staffApiRoutes = [
  ...
  "src/routes/api.os-automation-status.ts",
...
for (const path of staffApiRoutes) {
  ...
  forbidText(source, "OPENAI_API_KEY", path);
  forbidText(source, "GEMINI_API_KEY", path);
  forbidText(source, "META_PAGE_ACCESS_TOKEN", path);
}

const publishRoute = await text("src/routes/api.internal.publish-worker.ts");
requireText(publishRoute, "verifyInternalWorkerRequest", "publish worker route");
requireText(publishRoute, "processOnePublishJob()", "publish worker route");
forbidText(publishRoute, "resolveStaffSession", "publish worker route");
forbidText(publishRoute, "SUPABASE_SECRET_KEY", "publish worker route");
forbidText(publishRoute, "getPublishingProvider", "publish worker route");
forbidText(publishRoute, "claim_next_publish_job", "publish worker route");

const publishProcessor = await text("src/platform/publish-worker.server.ts");
for (const needle of [
  "supabaseSecretRpc",
  ...
  'rpcJson("claim_next_publish_job")',
  'rpcJson("defer_publish_job"',
  'rpcJson("complete_publish_job"',
  'rpcJson("fail_publish_job"',
  "ProviderResultSchema",
]) {
  requireText(publishProcessor, needle, "publish worker processor");
}
for (const needle of [
  "resolveStaffSession",
  "serviceRoleRpc",
  ...
  forbidText(publishProcessor, needle, "publish worker processor");
}
...
for (const needle of [
  ...
  "AI_TEXT_MODEL=qwen3.7-max",
  "AI_IMAGE_MODEL=wan2.7-image-pro",
  "AI_VIDEO_TEXT_MODEL=wan2.7-t2v-2026-06-12",
  "AI_VIDEO_IMAGE_MODEL=wan2.7-i2v-2026-04-25",
]) {
  requireText(envExample, needle, "environment template");
}
...
for (const needle of ["for update", "background_jobs", "publish_content", "scheduled_for"]) {
  requireText(transitionMigration, needle, "content transition migration");
}
...
for (const needle of [
  ...
  requireText(workerMigration, needle, "publish worker migration");
}
...
for (const needle of [
  ...
  "content_edited_review_required",
  "status = 'needs_review'",
  "scheduled_for = null",
]) {
  requireText(editMigration, needle, "content edit migration");
}

const crmWorkflowMigration = (
  await text("supabase/migrations/20260708_000019_add_staff_crm_workflow_rpc.sql")
).toLowerCase();
for (const needle of ["for update", "do_not_contact", "follow_up_limit_reached", "audit_logs"]) {
  requireText(crmWorkflowMigration, needle, "CRM workflow migration");
}
...
for (const needle of [
  ...
  requireText(mediaMigration, needle, "AI media migration");
}
...
for (const needle of [
  ...
  "storage.foldername(name)",
  "auth.uid()::text",
  "public.can_manage_relax_fix_media()",
]) {
  requireText(mediaPolicyFix, needle, "AI media policy fix");
}
...
for (const needle of [
  "get_staff_video_generation_jobs",
  "security definer",
  "set search_path = public, pg_temp",
  "limit 100",
  "from public, anon",
  ...
  requireText(videoJobsMigration, needle, "video jobs read migration");
}
...
for (const needle of [
  ...
  'all("SUPABASE_SECRET_KEY", "INTERNAL_WORKER_TOKEN")',
  "alibabaQwenProvider",
  "alibabaWanImageProvider",
  "alibabaWanVideoProvider",
  "getVideoProviderById",
]) {
  requireText(providerRegistry, needle, "provider registry");
}
...
for (const needle of [
  ...
  'type: "first_frame"',
  "usesSourceImage ? {} : { ratio:",
  "/api/v1/tasks/",
]) {
  requireText(alibabaAdapter, needle, "Alibaba adapter");
}

const mediaStorage = await text("src/platform/media-storage.server.ts");
for (const needle of [
  'MEDIA_BUCKET = "relax-fix-media"',
  ...
  "authenticatedObjectExists(path, accessToken)",
]) {
  requireText(mediaStorage, needle, "media storage");
}
...
for (const needle of [
  ...
  requireText(contentStudio, needle, "Content Studio");
}
