...
requireText(worker, "serviceRoleRpc", "publish worker");
...
const serviceRole = await text("src/platform/service-role.server.ts");
requireText(serviceRole, "process.env.SUPABASE_SERVICE_ROLE_KEY", "service role client");
requireText(serviceRole, "Authorization: `Bearer ${key}`", "service role client");
...
const migrations = new Map([
  ...
  ["supabase/migrations/20260708_000011_add_staff_inbox_read_rpcs.sql", ["get_staff_inbox", "get_staff_conversation_messages"]],
  ["supabase/migrations/20260708_000012_add_staff_conversation_mode_rpc.sql", ["set_staff_conversation_mode"]],
  ["supabase/migrations/20260708_000013_add_staff_generated_content_rpc.sql", ["create_staff_generated_content_batch"]],
  ["supabase/migrations/20260708_000014_add_staff_os_read_models.sql", ["get_staff_content_items", "get_staff_growth_analytics", "get_staff_command_center"]],
...
const transitionMigration = (await text("supabase/migrations/20260708_000016_add_staff_content_review_schedule_rpc.sql")).toLowerCase();
...
const workerMigration = (await text("supabase/migrations/20260708_000017_add_publish_worker_rpcs.sql")).toLowerCase();
...
console.log(`AI OS contract verification passed (${checks} assertions across ${osPages.length} OS pages).`);
