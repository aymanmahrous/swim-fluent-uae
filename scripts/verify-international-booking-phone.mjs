...
let unsafeCombinedMigrationExists = true;
try {
  await access(join(root, "supabase/migrations/20260711_000031_international_booking_phones.sql"));
} catch {
  unsafeCombinedMigrationExists = false;
}
verify(!unsafeCombinedMigrationExists, "Phase B must not contain the unsafe combined migration");

console.log(`International booking phone application verification passed (${checks} assertions).`);
