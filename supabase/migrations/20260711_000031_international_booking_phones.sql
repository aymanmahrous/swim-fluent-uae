-- International phone support for public booking requests.
-- This migration is intentionally additive/non-destructive: it does not rewrite or delete existing bookings.
-- Application-level semantic validation uses libphonenumber-js before this RPC boundary;
-- PostgreSQL stores the canonical E.164 digits and enforces its structural length/shape.
...
begin
  ...
  elsif v_digits ~ '^9715[0-9]{8}$' then
    null;
  else
    return null;
  end if;
...
-- Compatibility wrapper for existing database callers. It remains UAE-specific by name.
...
as $$
declare
  v_phone text := public.normalize_international_phone(p_phone);
begin
  if v_phone ~ '^9715[0-9]{8}$' then
    return v_phone;
  end if;
  return null;
end;
...
revoke all on function public.normalize_international_phone(text) from public, anon, authenticated;
revoke all on function public.normalize_uae_phone(text) from public, anon, authenticated;
...
-- RLS remains enabled and no table privilege is widened.
