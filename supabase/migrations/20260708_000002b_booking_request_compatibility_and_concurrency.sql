-- Compatibility-safe activation for booking_requests and submit_booking_request.
-- Safe for a missing table or an already-compatible table. Aborts transaction on incompatible schema.
...
begin
  ...
  if v_missing is not null then
    raise exception 'booking_requests exists with incompatible schema. Missing columns: %', v_missing;
  end if;
end
$$;
...
create unique index if not exists booking_requests_active_phone_slot_unique
  on public.booking_requests(normalized_phone, requested_date, requested_time)
