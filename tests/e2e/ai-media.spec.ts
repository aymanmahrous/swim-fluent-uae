...
test("AI Image and AI Video persist privately and reload from Media Library", async ({
  page,
  browser,
}) => {
  test.setTimeout(25 * 60 * 1000);
  const [primary, secondary] = (await readCredentials()).users;
  const imagePrompt = `Relax Fix UAE E2E image ${randomUUID()} premium swimmer training in Abu Dhabi, realistic editorial sports photography`;
  const videoPrompt = `Relax Fix UAE E2E video ${randomUUID()} cinematic swimmer training sequence in a premium Abu Dhabi pool, realistic motion`;
